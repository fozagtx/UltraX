...
export default async function OverviewPage({
  ...
  const [kpis, stable24h, whales, largest24h, holderCache, series] =
    await Promise.all([
  ...
  const whaleKpi = metrics["whale-transfers"] as
    | { latest?: { amountUsd?: number } | null }
    | undefined;
  ...
  const feedItems = feedIsFallback
    ? (largest24h as WhaleItem[])
    : whaleItems;
  ...
  const cards: {
    ...
    { id: "new-addresses", label: "New addresses", ...numEntry("new-addresses", compact) },
    ...
    },
    {
      ...
      caption:
        metrics["stablecoin-volume"]?.latest != null
          ? metrics["stablecoin-volume"].partial && metrics["stablecoin-volume"].asOfDate
    ...
    },
    {
      ...
      caption:
        metrics["stablecoin-netflow"]?.latest != null
          ? metrics["stablecoin-netflow"].partial && metrics["stablecoin-netflow"].asOfDate
  ...
  return (
    <div className="space-y-6">
      ...
      {stable24h ? (
        <section
          className="flex flex-wrap gap-x-8 gap-y-2 rounded-lg border px-4 py-3 text-sm"
          style={{ background: "var(--surface)", borderColor: "var(--border)" }}
        >
          <span className="text-xs" style={{ color: "var(--muted)" }}>
            last 24h (rolling)
          </span>
          <span className="mono">volume {usd(stable24h.volumeUsd)}</span>
          <span className="mono">netflow {usd(stable24h.netflowUsd)}</span>
          <span className="mono">
          ...
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <div className="flex flex-wrap gap-1">
              {SERIES_IDS.map((id) => (
                <Link
                  key={id}
                  href={`/?m=${id}&range=${range}`}
                  className="px-2 py-0.5 text-xs"
                  style={{
                    color:
                      id === metricId ? "var(--accent)" : "var(--muted)",
                    borderBottom:
                      id === metricId
            ...
            <div className="flex gap-1">
              {[7, 30].map((d) => (
                <Link
                  key={d}
                  href={`/?m=${metricId}&range=${d}`}
                  className="mono px-2 py-0.5 text-xs"
                  style={{
                    color: d === range ? "var(--accent)" : "var(--muted)",
                    borderBottom:
                      d === range
                        ? "1px solid var(--accent)"
                        : "1px solid transparent",
